...
class Retrace:
    ...
    def __call__(
        ...
        Returns:
            ...
            L = 𝔼_τ[(Q_t - Q_ret_t)^2]
            Q_ret_t = r_t + γ * (𝔼_π_target [Q(s_t+1,•)] + c_t+1 * Q_π_target(s_t+1,a_t+1))
            + γ * c_t+1 * Q_ret_t+1.
            with trajectory τ = {(s_0, a_0, r_0),..,(s_k, a_k, r_k)}
        The recursive form for calculating Q_ret can be found in
        ...
        assert (
            Q.shape
            == target_Q.shape
            == expected_target_Q.shape
            == rewards.shape
            == target_policy_probs.shape
        )
        ...
        with torch.no_grad():
            ...
            c_ret = self.calc_retrace_weights(
            ...
            for t in reversed(range(1, T)):
                Q_ret[:, t - 1] = (
                    rewards[:, t - 1]
                    + gamma * c_ret[:, t] * (Q_ret[:, t] - target_Q[:, t])
    ...
    def calc_retrace_weights(target_policy_logprob, behaviour_policy_logprob):
        ...
        Returns:
            retrace weights c
        """
        log_retrace_weights = (target_policy_logprob - behaviour_policy_logprob).clamp(
            max=0
        )
        retrace_weights = log_retrace_weights.exp()
        assert not torch.isnan(
            log_retrace_weights
        ).any(), "Error, a least one NaN value found in retrace weights."

...
class SimpleBuffer:
    ...
    def __init__(self, state_dim, action_dim, device, size=2000000):
        """
        Initialize replay buffer. SimpleBuffer simply uses numpy arrays to store data from gym
        environments.
    ...
    def store(self, s, s_next, act, rew, log_p, done):
        """
        Stores a complete step with actor information. For compatibility reasons log_p will be
        offered as a storage option but disregarded since no algorithm using SimpleBuffer requires
        it.

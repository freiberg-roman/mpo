from mpo.core.q_loss.q_loss_fn import UpdateQ_TD, UpdateQRetrace
from mpo.core.policy_loss.loss_mpo import PolicyUpdateNonParametric
from mpo.core.helper_fn import Sampler, TestAgent, TargetActionMPO
import gym
from mpo.core.main_loop import runner
from mpo.core.nets.nets_mpo_double_q import MLPActorCritic
from mpo.core.nets.nets_mpo_single_q import MLPActorCriticSingle
from copy import deepcopy
import itertools
import torch
from mpo.common.tray_dyn_buf import DynamicTrajectoryBuffer
from mpo.common.simple_buffer import SimpleBuffer
from mpo.core.builder.environment_settings import episode_len


def mpo_non_parametric_td0(
    env_name,
    local_device,
    writer,
    lr_pi=5e-4,
    ...
    """
    Builds a runnable version of the MPO non-parametric version using double Q functions in
    combination with TD0 for updating Q values
    ...
    @param min_steps_per_epoch: minimal amount of steps that will be sampled during one iteration
        of learning
    ...
    @param update_after: amount of updates after which the target model will be copied over
        (only for policy)
    ...
    replay_buffer = SimpleBuffer(state_dim=ds, action_dim=da, device=local_device)
    ...
    actor_step = TargetActionMPO(device=local_device, ac_targ=ac_targ, ds=ds)
...
def mpo_non_parametric_retrace(
    env_name,
    local_device,
    writer,
    lr_pi=5e-4,

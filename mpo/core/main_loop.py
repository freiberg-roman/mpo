...
def runner(
    ...
    """
    This is a blanc implementation of the sample and update cycles shared between the implemented
    algorithms. It is required that all functions manage their data on their own by having state.
    ...
    @param test_after: the minimal amount of steps that will be performed before an evaluation of
        the model through the test_agent function
    @param update_steps: amount of updates before next sampling stage
    @param update_after: amount of updates before updating the target networks according to
        target_update
    """
    ...
    while buffer.stored_interactions() < total_steps:
        ...
        if buffer.stored_interactions() - current_steps >= test_after:
            ...
            with torch.no_grad():
                test_agent(it)
            writer.add_scalar("performed_steps", buffer.stored_interactions(), it)

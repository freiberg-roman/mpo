...
class GaussianMLPActor(nn.Module):
    ...
    def forward(self, state):
        ...
        action_high = torch.from_numpy(self.env.action_space.high)[None, ...].to(
            self.device
        )
...
class MLPActorCriticSingle(nn.Module):
    def __init__(
        ...
        env,
        device,
        hidden_sizes_pi=(256, 256),
        hidden_sizes_q=(256, 256),
        activation=nn.ReLU,
    ):
        super().__init__()
    ...
    def get_logp(mean, std, action, expand=None):
        if expand is not None:
            dist = Independent(Normal(mean, std), reinterpreted_batch_ndims=1).expand(
                expand
            )
        else:

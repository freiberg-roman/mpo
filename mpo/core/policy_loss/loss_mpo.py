...
def gaussian_kl(targ_mean, mean, targ_std, std):
    ...
    inner_cov = (
        torch.log(cov.prod(-1) / targ_cov.prod(-1)) - n + (cov_inv * targ_cov).sum(-1)
    )
...
def dual(eta, targ_q, eps_dual):
    ...
    return (
        ...
        + torch.mean(max_q)
        + eta
        * torch.mean(torch.log(torch.mean(torch.exp((targ_q - max_q) / eta), dim=0)))
    )
...
class PolicyUpdateNonParametric:
    ...
    def __call__(self):
        ...
        # M-step
        qij = torch.softmax(
            targ_q / F.softplus(self.eta).item(), dim=0
        )  # (M, B) or (da, B)
        ...
        c_mean, c_cov = gaussian_kl(targ_mean=b_μ, mean=mean, targ_std=b_A, std=std)
        ...
        self.writer.add_scalar(
            "combined_loss", loss_l.item() - loss_eta.item(), self.run
        )
...
class UpdateLagrangeTrustRegionOptimizer:
    ...
    def __init__(self, writer, eta_mean, eta_cov, eps_mean, eps_cov, lr_kl):
        """

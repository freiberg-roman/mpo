import torch
from mpo.common.retrace import Retrace


class UpdateQRetrace:
    """
    Encapsulates Retrace algorithm for Q function updates such that on call one update cycle is
    performed.
    """

    def __init__(
        self, writer, critic_optimizer, ac, ac_targ, buffer, batch_size, gamma, device
    ):
        """
    ...
    def __call__(self):
        ...
        targ_act_logp = self.ac_targ.get_logp(
            targ_mean, targ_chol, samples["action"]
        ).unsqueeze(-1)
...
class UpdateQ_TD:
    """
    Encapsulates TD0 algorithm for Q function updates such that on call one update cycle is
    performed.
    """

    def __init__(
        self, writer, critic_optimizer, ac, ac_targ, buffer, batch_size, gamma, entropy
    ):
        """
        Initializes needed values
    ...
    def __call__(self):
        ...
        with torch.no_grad():
            ...
            backup = samples["reward"] + self.gamma * (1 - samples["done"]) * (
                targ_q - self.entropy * logp
            )

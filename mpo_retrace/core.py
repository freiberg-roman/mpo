...
class GaussianMLPActor(nn.Module):
    ...
    def forward(self, state):
        ...
        mean = torch.tanh(mean) * self.act_limit  # scaling on action dimension
        std = self.std_layer(net_out)
        std = F.softplus(std)
        return mean, std

    def get_logp(self, mean, std, pi_action):
        # invert scaling
        pi_dist = Independent(Normal(mean, std), reinterpreted_batch_ndims=self.act_dim)

        return pi_dist.log_prob(pi_action)

    def get_act(self,
                mean,  # (..., act_dim)
                std, deterministic=False):
        pi_dist = Independent(Normal(mean, std), reinterpreted_batch_ndims=self.act_dim)
        if deterministic:
            return mean, None
        else:
